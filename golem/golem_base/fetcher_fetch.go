...
/*
*/ 
func (f *Fetcher) FormatVariablesForDelta(start int, end int) map[string][]float64 {
	...
	for i := start; i < end; i++ {
		for _, v_ := range f.analysis[i] {
			output[v_.varName] = append(output[v_.varName], v_.varValue)
		}
	}
...
func (f *Fetcher) CalculateDelta(deltaOp string, start int, end int) []*Variable {
	...
	for _, k := range keysToIterate {
		...
		variable := &Variable{varName: vn, varValue: DeltaOpOnSequence(m[k], deltaOp), spareValue: deltaOp}
